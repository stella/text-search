...
import { classifyPatterns, countAlternations } from "../src/classify";
...
describe("countAlternations", () => {
  ...
  test("large alternation", () => {
    const alt = Array.from({ length: 80 }, (_, i) => `title${i}`).join("|");
    expect(countAlternations(alt)).toBe(80);
  });

  test("nested groups", () => {
    expect(countAlternations("(a|(b|c))|d")).toBe(2);
  });
});
...
describe("classifyPatterns", () => {
  ...
  test("named pattern", () => {
    const result = classifyPatterns([{ pattern: "x|y", name: "test" }]);
    expect(result[0]!.alternationCount).toBe(2);
