...
const config = {
  ...
  labels: [...DEFAULT_ENTITY_LABELS, "date of birth"],
  workspaceId: "bench",
};
...
for (const gf of goldFiles) {
  ...
  let text;
  try {
    text = await Bun.file(`${CORPUS}/${base}.txt`).text();
  } catch {
    continue;
  }
  ...
  for (let pi = 0; pi < predicted.length; pi++) {
    ...
    for (let gi = 0; gi < gold.length; gi++) {
      ...
      const os = Math.max(p.start, g.start),
        oe = Math.min(p.end, g.end);
      if (oe <= os) continue;
      const shorter = Math.min(p.end - p.start, g.end - g.start);
      if (oe - os >= shorter * 0.5) {
        goldMatched.add(gi);
        predMatched.add(pi);
        break;
      }
  ...
  const ltp = goldMatched.size,
    lfp = predicted.length - predMatched.size,
    lfn = gold.length - goldMatched.size;
  tp += ltp;
  fp += lfp;
  fn += lfn;
  const p = ltp + lfp > 0 ? ltp / (ltp + lfp) : 0,
    r = ltp + lfn > 0 ? ltp / (ltp + lfn) : 0;
  const f1 = p + r > 0 ? (2 * p * r) / (p + r) : 0;
  const status = f1 >= 0.8 ? "OK" : f1 >= 0.5 ? "WARN" : "FAIL";
  console.log(
    `  [${status}] ${base.slice(0, 55).padEnd(55)} P=${(p * 100).toFixed(0).padStart(3)}% R=${(r * 100).toFixed(0).padStart(3)}% F1=${(f1 * 100).toFixed(0).padStart(3)}% ${ms.toFixed(0).padStart(5)}ms (${ltp}/${ltp + lfn} gold, ${lfp} FP)`,
  );
}
